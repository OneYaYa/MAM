...
def discussion_and_voting(roles, question, file_name, modality_type, type_name, history_item):
    ...
    for round_number in range(1, max_rounds + 1):
        ...
        agree_votes, disagree_votes = moderator.vote(roles, question, file_name, modality_type, type_name, record_discussions)
...
def roles_meeting(question, file_name, modality_type, type_name, roles_generated, history_item):
    ...
    print(roles)
    if roles == []:
        roles=["Radiologic Technologist","Specialist Doctor","General Practitioner"]
...
def roles_meeting(question, file_name, modality_type, type_name, roles_generated, history_item):
    ...
    roles = extract_roles_from_text(roles_generated)
    if not roles:
        # 创建默认的 Role 对象而不是字符串列表
        default_roles_data = [
            ("General Practitioner", "- Perform initial patient assessment\n- Coordinate care between specialists\n- Provide overall medical evaluation"),
            ("Radiologic Technologist", "- Perform and analyze imaging studies\n- Ensure proper imaging protocols\n- Assess image quality"),
            ("Specialist Doctor", "- Provide specialized medical assessment\n- Interpret clinical findings\n- Recommend treatment options")
            
        ]
        
        roles = []
        for name, responsibilities in default_roles_data:
            roles.append(Role(name, responsibilities))
